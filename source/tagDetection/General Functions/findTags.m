function tags = findTags(tagData, preTagData, postTagData, nBits, nTags, mirrored)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
%         disp('No data pulses found, not a tag, at least not a valid one.')
    ...
    if mirrored && mod(numDataPulses, 2) ~= 0
        disp('Tag is invalid - not mirrored');
        continue;
    end
    dataPulseIdx = firstDataPulseIdx:lastDataPulseIdx;
    try
    ...
    %         fprintf('Total data size (pw) = %f\n', (dataEnd - dataStart)/tagStruct.pulseWidth);
            ...
            % We're looking for unmirrored tags here
            endTime = dataEnd + tagStruct.pulseWidth*2;
            tagData = getTagBinaryID(risingEdgeTimes(dataPulseIdx), startTime, endTime, tagStruct);
            tagId = convertTagDataToId(tagData);
        end
